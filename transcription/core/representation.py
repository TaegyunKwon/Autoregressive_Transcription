...
def convert2onsets_and_frames(label, rep_type):
    if rep_type in ['base', 'four']:
        ...
        padded_frame = th.zeros((frames.shape[0] + 1, frames.shape[1]))
        ...
        frames = label == 1
        padded_frames = th.zeros(frames.shape[0] + 1, frames.shape[1])
        padded_frames[1:] = frames
        diff = padded_frames[1:] - padded_frames[:-1]
        onsets = diff == 1
        offsets = diff == -1
        return onsets, offsets, frames
    else:
